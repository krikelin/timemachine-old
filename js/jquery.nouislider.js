									...
									return this.each(function(){
										...
										knobs.bind('mousedown.noUiSlider',function(e){
											...
											$(document).bind('mousemove.noUiSlider', function(f){
												...
												// CHANGED BY drsounds@gmail.com to match the app
												/* lower knob */
												...
												if(knob.hasClass('noUi_lowerHandle')||!status){
													...
													if(status){
														var l=(parseInt(knob.parent().find('.noUi_upperHandle').css('left'))-knobCorrection)+7;
														if(flattened>l){
															flattened=l;
															knob.parent().find('.noUi_lowerHandle').css({'z-index':12});
															knob.parent().find('.noUi_upperHandle').css({'z-index':6});
														} else {
															knob.parent().find('.noUi_lowerHandle').css({'z-index':6});
															knob.parent().find('.noUi_upperHandle').css({'z-index':6});
														}
													}
												...
												if(knob.hasClass('noUi_upperHandle')||!status){
													...
													if(status){
														var l=(parseInt(knob.parent().find('.noUi_lowerHandle').css('left'))+knobCorrection)-7;
														
														if(flattened<l){
															flattened=l;
															knob.parent().find('.noUi_upperHandle').css({'z-index':12});
															knob.parent().find('.noUi_lowerHandle').css({'z-index':6});
